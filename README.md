...
where α (volatility level), β (elasticity parameter), ρ (correlation), and ρ (volatility of volatility) are calibrated. Here Fₜ stands for forward price or underlying asset price, σₜ means instantaneous volatility of Fₜ, Wₜ and Zₜ are Brownian motions.
...
RMSE = sqrt( (1/N) Σᵢ (σ_model(Kᵢ) − σ_market(Kᵢ))² )
